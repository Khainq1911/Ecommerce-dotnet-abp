...
$jobs += Start-Job -Name "DbMigrator" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/ECommerce.DbMigrator")
    dotnet run
...
$jobs += Start-Job -Name "DevCert" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../src/ECommerce.HttpApi.Host")
    dotnet dev-certs https -v -ep openiddict.pfx -p 66ab7c55-d28e-41ed-b714-d9a7db3d9f94
